...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            gt_corners = tools.compute_box_corners(label.box.center_x, 
                                                label.box.center_y, 
                                                label.box.width, 
                                                label.box.length, 
                                                label.box.heading)
            ...
            for (_, x, y, z, _, w, l, yaw) in detections:
                ...
                gt_bbox = Polygon(gt_corners)
                det_bbox = Polygon(det_corners)

                intersection = gt_bbox.intersection(det_bbox).area
                union = gt_bbox.union(det_bbox).area
